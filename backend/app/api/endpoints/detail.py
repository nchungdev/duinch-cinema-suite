...
async def _build_detail(client, tmdb_id: int, media_type: str):
    ...
    metadata = {
        ...
        "year": (raw.get("release_date") or raw.get("first_air_date") or "0000")[:4],
        ...
        "category": [{"name": g.get("name", "Other")} for g in raw.get("genres", [])],
        "actor": [p.get("name", "Unknown") for p in raw.get("credits", {}).get("cast", [])[:10]],
        "tmdb_id": tmdb_id,
